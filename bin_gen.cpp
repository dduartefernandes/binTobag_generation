...
{ 
  ...
  public:
    static uint64_t NFrame_Seq;  
    kitti4ROS(int argc, char** argv, uint64_t value, uint64_t value_):ros_rate_(value), number_frames(value_){   
      pub_ = n.advertise<sensor_msgs::PointCloud2> (argv[2], 1);
      //sub_ = n.subscribe("/p12_detections", 1, &kitti4ROS::cloud_cb, this);
      path_= argv[4];
      frames_id.open(argv[3], std::ios::in);
    }
    ~kitti4ROS(){
      if(frames_id.is_open())
        frames_id.close();
    }
    ...
    std::string get_sequence_path(void)
    {
      ...
      if(!frames_id.is_open()){
        ROS_INFO("Error while opening file related to frames id!");
        abort();
      }else{
        int x=0;
        while(frames_id.getline(line, 10)){          
          //if(x==NFrame_Seq)
          //{ 
            sf=path_;
            sf.append("/");
            ...
            ss_aux<<line;
            ss_aux>>NFrame_Seq;
            std::cout<<"[" << NFrame_Seq << "]: " << sf <<std::endl;
            return sf; 
          //}
          //x++;
        }
...
{ 
 ...
 if (argc!=5)
 {
   std::cout<<"Command example ./bin_gen 1 /p12_bag eval.txt /home/p12/final_image_dpu/p12_server/Vitis/Vitis-AI/pointpillars/data/KITTI/training/velodyne" << std::endl;
   abort();
   /* code */
 }
